  ...
  try {
    ...
    const secondDegreeConnections = await prisma.connection.findMany({
      where: {
        ...
        AND: {
          toId: {
            notIn: [...firstDegreeIds, userId], // Exclude 1st degree and self
          }
        }
      },
      select: {
    ...
    // Get 3rd degree connections
    const thirdDegreeConnections = await prisma.connection.findMany({
      where: {
        fromId: { in: secondDegreeIds },
        AND: {
          toId: {
            notIn: [...firstDegreeIds, ...secondDegreeIds, userId], // Exclude 1st, 2nd degree and self
          }
        }
      },
      select: {
    ...
    const recommendations = [
      ...
      // Sort by degree first (2nd before 3rd)
